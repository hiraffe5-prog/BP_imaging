function [ rour4dB, roua4dB,  max_Pr, max_Pa, Ia , Ir] = Reso_PSLR_ISLR_4dB(Ga_in,range_pixel,azimuth_pixel,upr,upa,flag)
...
for i=1:(Na-AI-3)
    ...
    if  Ga(AI+i+1,RI) >= max_val_4dB &&  Ga(AI+i+2,RI) < max_val_4dB
        side_down=AI+i+1;
        break;
    end
end
...
%%%%%%%求方位分辨率%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
roua4dB=abs(side_up-side_down)*azimuth_pixel;

%%%%%%%求距离向积分旁瓣比%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:(Nr-RI-3)
    %if Ga(AI,RI+i) > Ga(AI,RI+i+1) & Ga(AI,RI+i+1) < Ga(AI,RI+i+2)      %%%直到找到第一个旁瓣
    if Ga(AI,RI+i+1) >= max_val_4dB &&  Ga(AI,RI+i+2) < max_val_4dB 
        side_right=RI+i+1;
        break;
    end
end
...
%%%%%%求距离分辨率%%%%%%%%%%%%%%%%%%%%
rour4dB=abs(side_right-side_left)*range_pixel;
